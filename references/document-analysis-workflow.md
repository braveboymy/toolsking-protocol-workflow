...
**陷阱 7：表具行业敏感数据被压缩为 raw_hex（高危 —— 一票否决）**

这是表具协议（燃气表/水表/热量表）接入中**最严重的错误**。
协议文档中的累积量、金额、流量、压力、温度等计量数据，如果因为它们来自 TLV 变长数据域
就被压缩成 `raw_hex`，会导致上位机完全无法展示、记录、校验这些核心业务数据——
而这恰恰是表具协议存在的根本目的。

**触发信号**：
- 协议文档中出现 `累计用气量`、`剩余金额`、`瞬时流量`、`压力`、`温度`、`声速` 等字段描述
- 协议文档中出现 `事件记录`、`事件日志`、`历史事件`、`告警记录` 等字段描述
- 分析者因为"这是 TLV 里的可选字段，数量多、顺序不固定"，直接用了 `raw_hex + dynamic: true`
- 在 commands.yaml 中看到 `type: raw_hex` + `description: Tag 0xXX ...` 的模式

**实际危害**：
- 上位机 GUI 的"实时数据"页显示一串 hex 字符串而非"累计用气量: 123.45 m³"
- 历史数据查询结果无法按数值排序/筛选/绘图
- 报警联动无法基于数值判断（如余额 < 阈值 → 关阀）
- 数据库存储的是 hex 字符串，无法做 SUM/AVG 等聚合查询

**正确处理**：详见 `commands-yaml-template.md` **第 13 节**，核心要点：
1. 计量数据是"一等公民"，每一条都必须独立定义为字段（§13.1）
2. 有明确的触发词列表——命中任一即强制独立定义（§13.3）
3. 变长数组型计量数据（如 BYTE[4*N] 的历史累计用量）也不能压缩，需按 §13.5 处理
4. 透传数据、固件包等非计量数据可以合理使用 raw_hex（§13.6）
